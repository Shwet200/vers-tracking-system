////this is just to store the old endpoints for safety
//
...
//    })).filter(d => !isNaN(d.currentDensity) && !isNaN(d.voltage));
//
//    // Extract unique current densities
//    const uniqueCurrentDensities = [...new Set(polCurveDataDIW.map(d => d.currentDensity))].sort((a, b) => a - b);
//
//    console.log('Unique Current Densities:', uniqueCurrentDensities);
//
...
//    };
//
//    // Process the initial set of pol curve data from columns P, Q, R, etc.
...
//        currentDensity: ivData.slice(1).map(row => parseFloat(row[2])).filter(value => !isNaN(value)),
//        voltage: ivData.slice(1).map(row => parseFloat(row[3])).filter(value => !isNaN(value)),
//      });
//    }
//
...
//});
//
//////perfect but needs BoL data also
////router.post('/excel-analyzer', upload.single('excelFile'), async (req, res) => {
////  try {
////    if (!req.file) {
////      return res.status(400).send('No file uploaded');
...
////    };
////
////    // Return the processed data including unique current densities
////    res.json({
////      message: 'Excel file processed successfully',
////      dataAvailable: true,
////      counts,
...
////
////    console.log('Unique Current Densities:', uniqueCurrentDensities);
////    console.log('All Current Densities:', allCurrentDensities);
////
////    const responseData = {
...
//
//
//router.get('/api/engineer/durability-polcurve-diw', async (req, res) => {
...
//    }
//
//    console.log('Full additionalPolCurves data:', JSON.stringify(additionalPolCurves, null, 2));
//
//    const voltageVsTimeData = additionalPolCurves.map(curve => {
//      console.log(`Processing curve at time ${curve.hours}`);
//
//      const index = curve.currentDensity.findIndex(cd => {
//        console.log(`Comparing: ${cd} with ${selectedCurrentDensity}`);
//        return Math.abs(cd - selectedCurrentDensity) < 0.0001;
//      });
//
//      if (index !== -1) {
...
//          .map(row => ({
//            currentDensity: parseFloat(row[currentDensityCol.charCodeAt(0) - 'A'.charCodeAt(0)]),
//            voltage: parseFloat(row[voltageCol.charCodeAt(0) - 'A'.charCodeAt(0)])
//          }))
//          .filter(d => !isNaN(d.currentDensity) && !isNaN(d.voltage));
//
//        additionalPolCurves.push({
...
//    }
//
//    // Process additionalEISData
//    let baseEISColumnIndex = 'V'.charCodeAt(0) - 'A'.charCodeAt(0); // Start at column 'V'
...
//        const reZCol = getColumnLetter(currentColumnIndex + 2); // X or AH, etc.
//        const imZCol = getColumnLetter(currentColumnIndex + 3); // Y or AI, etc.
//
//        const pairedData = eisHfrData.slice(1)
//          .map(row => ({
//            reZ: parseFloat(row[reZCol.charCodeAt(0) - 'A'.charCodeAt(0)]),
//            imZ: parseFloat(row[imZCol.charCodeAt(0) - 'A'.charCodeAt(0)])
//          }))
...
//      console.log(`Processing curve at time ${curve.hours}`);
//
//      // Ensure that curve.data exists and has the expected structure
