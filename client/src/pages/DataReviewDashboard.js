...
const DataReviewDashboard = () => {
  ...
  useEffect(() => {
    const fetchCurrentDensities = async () => {
      try {
        const response = await axios.get('/api/engineer/polcurve-diw');
        console.log('Current Densities Response:', response.data); // Log the full response
        const currentDensities = response.data.scatterData.map(d => d.x);
        console.log('Mapped Current Densities:', currentDensities); // Log the mapped current densities
        const options = currentDensities.map(cd => ({ key: cd, text: cd, value: cd }));
        console.log('Dropdown Options:', options);  // Check what options are being set
    ...
    if (dropdown1 && dropdown2) {
      ...
      if (dropdown1 === 'Pol Curve' && dropdown2 === 'DIW') {
        if (selectedCurrentDensity) {
          endpoint = `/api/engineer/polcurve-diw-time?currentDensity=${selectedCurrentDensity}`;
          xAxisLabel = 'Time (hr)';
          yAxisLabel = 'Voltage (V)';
        } else {
        ...
        <Grid.Row>
          ...
          <Grid.Column width={6}>
            ...
            <Grid>
              ...
              {/* Conditional Dropdown for Current Density */}
